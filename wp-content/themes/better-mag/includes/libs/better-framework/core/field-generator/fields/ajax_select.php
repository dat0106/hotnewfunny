...
echo Better_Framework::html()->add( 'span' )->class( 'preloader' )->text( '<img src="'. includes_url( 'images/wpspin.gif' ) .'"/>' )->display();
...
if( !empty( $options['value'] ) )
    foreach( explode( ',', $options['value'] ) as $val ) {
        ...
        $sub = Better_Framework::html()->add( 'li' )->data( 'id', $val )->text( $name )->text( '<span class="del">x</span>' );
        $controls->text( $sub );
    }
