...
/**
 * Base Class for handle menu related functionality.
 *
 * TODO Fix menu item limit that happens because of "max_input_vars",
 * for fixing this collect all bf fields into one variable with JS and after that post!
 *
 */
class BF_Menus{
    ...
    public function save_menu_fields( $menu_id, $menu_item_db_id, $args ){
        ...
        foreach( $this->fields as $key => $field ){
            ...
            // add / update meta
            if( isset( $_POST['menu-item-' . $key] ) && is_array( $_POST['menu-item-' . $key] ) ){
                ...
                if( isset( $field['save-std'] ) && ! $field['save-std'] ){
                    if( $_POST['menu-item-' . $key][$menu_item_db_id] != $field[$std_id] )
