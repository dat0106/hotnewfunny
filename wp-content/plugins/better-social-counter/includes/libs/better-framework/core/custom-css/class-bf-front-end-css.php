...
class BF_Front_End_CSS extends BF_Custom_CSS {
    ...
    function register_custom_css(){
        ...
        $final_css = $this->prepare_final_css();

        if( isset( $final_css['fonts'] ) && ! empty( $final_css['fonts'] ) ){
            foreach( (array) $final_css['fonts'] as $key => $font ){
                if( $key == 0 ){
        ...
        if( ! empty( $final_css['css'] ) )
            wp_enqueue_style( 'better-framework-custom', get_site_url() . '/?better_framework_css=1&ver=' . get_transient( '__better_framework__final_fe_css_version' ), array(), NULL );
    ...
    function prepare_final_css(){
        ...
        // TODO add support when it used only in plugin and there is no theme
        if( ( $final_css_version = get_transient( '__better_framework__final_fe_css_version' ) ) !== false ){
        ...
        if( $final_css_version || ( false == ( $final_css = get_transient( '__better_framework__final_fe_css' ) ) ) ){
            $this->load_all_fields();

            $final_css = array();
            $final_css['css'] = $this->render_css();
            $final_css['fonts'] = $this->render_fonts( 'url' );

            set_transient( '__better_framework__final_fe_css' , $final_css );
            set_transient( '__better_framework__final_fe_css_version' , Better_Framework::theme()->get('Version') . '-' . time() );
        }
    ...
    function load_panel_fields(){
        ...
        //load from cache if available
        if( true == ( $cached_fields = get_transient( '__better_framework__panel_css') ) ){
            $this->fields = array_merge( $this->fields, $cached_fields );
            return;
        } else {
            $cached_fields = array();
        ...
        foreach( Better_Framework::options()->options as $panel_id => $panel_value ) {
            ...
            // load saved value
            $panel_saved_options = get_option( $panel_id );
        ...
        if( count( $cached_fields ) ){
            array_unshift( $cached_fields , array(
                ...
                'type'  => 'comment',
                array(
                    'comment' => " Panel Options Custom CSS"
                )
            ));
            $this->fields = array_merge( $this->fields , $cached_fields );

        }
        set_transient( '__better_framework__panel_css' , $cached_fields );
    }
                ...
                foreach( (array) $sidebar_widgets[$widget_id] as $widget_field_key => $widget_field_value){
                    ...
                    foreach( (array) $fields as $css_field ){
                        ...
                        if( $widget_field_key == $css_field['field'] ){
                            ...
                            if( BF_Widgets_General_Fields::is_general_field( $widget_field_key ) ){
                                if( apply_filters( 'better-framework/widgets/options/general/' . $widget_field_key . '/default' , '' ) == $widget_field_value )
                                    continue;
